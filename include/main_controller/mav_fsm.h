...
private:
  ...
  ros::Publisher local_pos_pub_, speed_pub_, att_pub_, desi_odom_pub_,
      trigger_pub_;
  ros::Timer mav_fsm_;

  dynamic_reconfigure::Server<dynamic_params::ControllerParamsConfig> srv;
  dynamic_reconfigure::Server<
      dynamic_params::ControllerParamsConfig>::CallbackType f;
  ...
  typedef enum { takeoff, attctr, land, waiting, trajectory } workstate_t;
...
*/
  ...
  void dynParamsCallback(dynamic_params::ControllerParamsConfig &config,
                         uint32_t level);

  void rcCallback(const mavros_msgs::RCInConstPtr &msg);

  void
  positionCmdCallback(const quadrotor_msgs::PositionCommand::ConstPtr &cmd_msg);
