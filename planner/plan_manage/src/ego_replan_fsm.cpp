...
namespace ego_planner {
    ...
    void EGOReplanFSM::init(ros::NodeHandle &nh) {
        ...
        if (planner_manager_->pp_.drone_id >= 1) {
            ...
            swarm_trajs_sub_ = nh.subscribe(sub_topic_name.c_str(), 10, &EGOReplanFSM::swarmTrajsCallback, this,
    ...
    void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp) {
        bool success = false;
        success = planner_manager_->planGlobalTraj(odom_pos_, odom_vel_, Eigen::Vector3d::Zero(), next_wp,
    ...
    void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp,const Eigen::Vector3d next_vel) {
        bool success = false;
        success = planner_manager_->planGlobalTraj(odom_pos_, odom_vel_, Eigen::Vector3d::Zero(), next_wp,
                                                   next_vel, Eigen::Vector3d::Zero());
    ...
    void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedPtr &msg) {
        ...
        Eigen::Quaterniond end_q(msg->pose.orientation.w, msg->pose.orientation.y, msg->pose.orientation.z,
                                 msg->pose.orientation.x);

        Eigen::Vector3d end_dir = end_q * Eigen::Vector3d::UnitX();
        Eigen::Vector3d end_vel = end_dir / 100;

        planNextWaypoint(end_wp,end_vel);
    }
    ...
    void EGOReplanFSM::BroadcastBsplineCallback(const traj_utils::BsplinePtr &msg) {
        ...
        if (msg->order % 2) {
            ...
            planner_manager_->swarm_trajs_buf_[id].duration_ =
                    (msg->knots[msg->knots.size() - floor(cutback)] + msg->knots[msg->knots.size() - ceil(cutback)]) /
                    2;
        }
        ...
        planner_manager_->swarm_trajs_buf_[id].start_pos_ = planner_manager_->swarm_trajs_buf_[id].position_traj_.evaluateDeBoorT(
                0);
    ...
    void EGOReplanFSM::swarmTrajsCallback(const traj_utils::MultiBsplinesPtr &msg) {
        ...
        {
            ROS_ERROR("Wrong trajectory size! msg->traj.size()=%d, msg->drone_id_from+1=%d", (int) msg->traj.size(),
                      msg->drone_id_from + 1);
            return;
        }
        ...
        for (size_t i = 0; i < msg->traj.size(); i++) {
            ...
            planner_manager_->swarm_trajs_buf_[i].start_pos_ = planner_manager_->swarm_trajs_buf_[i].position_traj_.evaluateDeBoorT(
                    0);
    ...
    void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e) {
        ...
        for (double t = t_cur; t < info->duration_; t += time_step) {
            if (t_cur < t_2_3 && t >=
                                 t_2_3) // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is considered valid and will get checked.
                break;
            ...
            for (size_t id = 0; id < planner_manager_->swarm_trajs_buf_.size(); id++) {
                ...
                Eigen::Vector3d swarm_pridicted = planner_manager_->swarm_trajs_buf_.at(
                        id).position_traj_.evaluateDeBoorT(t_X);
    ...
    void EGOReplanFSM::publishSwarmTrajs(bool startup_pub) {
        ...
        if (startup_pub) {
            ...
            if ((int) multi_bspline_msgs_buf_.traj.size() == planner_manager_->pp_.drone_id + 1) {
                ...
                ROS_ERROR("Wrong traj nums and drone_id pair!!! traj.size()=%d, drone_id=%d",
